<#
    .SYNOPSIS
    Runs a sequence of tests to determine the readiness of the environment for disabling TLS 1.0 and below
     
    .DESCRIPTION
    Runs a sequence of tests to determine the readiness of the environment for disabling TLS 1.0 and below. This should be run on every server that is to have TLS 1.0 disabled, and every client that connects to that server.
                  
    .EXAMPLE
    Full command: Test-Tls12Readiness
...
function Test-Tls12Readiness {
    ...
    $Report = Test-SqlTls12Readiness


    #region RDP hotfix
    $Report | Add-Member -MemberType NoteProperty -Name RdpSupportsTls12 -Value $false
    $OsVersion = [version](Get-WmiObject Win32_OperatingSystem).Version
    if ($OsVersion -ge [version]"6.2") {
        $Report.RdpSupportsTls12 = $true
    } elseif ($OsVersion -ge [version]"6.1") {
        $RdpHotfix = Get-WmiObject Win32_QuickFixEngineering -Filter "HotFixID LIKE 'KB3080079'"
        if ($RdpHotfix) {
            $Report.RdpSupportsTls12 = $true
        } else {
            $Report.UpdatesRequired += "RDP: Install hotfix from https://www.microsoft.com/en-us/download/details.aspx?id=49074"
        }
    } else {
        $Report.UpdatesRequired += "RDP: limited to TLS 1.0 for this OS version"
    }

    $Report.SupportsTls12 = $Report.SupportsTls12 -and $Report.RdpSupportsTls12
    #region RDP hotfix
